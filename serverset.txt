...
update_serverset() {
    ...
    cp "$0" "/tmp/serverset-backup-$(date +%Y%m%d).sh"
    
    # Download latest version (replace with your actual URL)
    if wget -q -O "/tmp/serverset-new.sh" "https://raw.githubusercontent.com/your-repo/serverset/main/serverset.sh"; then
        chmod +x "/tmp/serverset-new.sh"
        
        # Get new version
        NEW_VERSION=$(grep "SCRIPT_VERSION=" "/tmp/serverset-new.sh" | cut -d'"' -f2)
        ...
        if [ "$NEW_VERSION" != "$SCRIPT_VERSION" ]; then
            ...
            if [ "$confirm" = "yes" ]; then
                mv "/tmp/serverset-new.sh" "$0"
                log "ServerSet актуализиран до версия $NEW_VERSION!"
                log "Рестартирайте скрипта за да използвате новата версия."
                exit 0
            fi
        else
        ...
        error "Неуспешно изтегляне на актуализацията!"
    fi
    
    rm -f "/tmp/serverset-new.sh"
    read -p "Натиснете Enter за връщане..."
}
